...
//            JYLog(@"%@", objects[i]);
        ...
        if (objects[i] == nil) {
            hasNilObject = YES;
            JYLog(@"%s object at index %lu is nil, it will be     filtered", __FUNCTION__, i);
            #if DEBUG
...
- (void)safeAddObject:(id)obj {
    if (obj == nil) {
        JYLog(@"%s can add nil object into NSMutableArray", __FUNCTION__);
    } else {
...
- (void)safeRemoveObject:(id)obj {
    if (obj == nil) {
        JYLog(@"%s call -removeObject:, but argument obj is nil", __FUNCTION__);
        return;
    }
...
- (void)safeInsertObject:(id)anObject atIndex:(NSUInteger)index {
    if (anObject == nil) {
        JYLog(@"%s can't insert nil into NSMutableArray", __FUNCTION__);
    } else if (index > self.count) {
        JYLog(@"%s index is invalid", __FUNCTION__);
    } else {
...
- (id)safeObjectAtIndex:(NSUInteger)index {
    if (self.count == 0) {
        JYLog(@"%s can't get any object from an empty array", __FUNCTION__);
        return nil;
    }
    if (index >= self.count) {
        JYLog(@"%s index out of bounds in array", __FUNCTION__);
        return nil;
    }
...
- (void)safeRemoveObjectAtIndex:(NSUInteger)index {
    ...
    if (index >= self.count) {
        JYLog(@"%s index out of bound", __FUNCTION__);
        return;
    }
